...
public class Skill1BaseMealPlan implements Skill {
    ...
    public String execute(Map<String, Object> params) {
        ...
        try {
            if (newDishName != null && !newDishName.trim().isEmpty() && dayIndex != null && mealIndex != null) {
                ...
                if (dishInfo != null && !dishInfo.isEmpty()) {
                    return generatePlanWithCustomDish(peopleCount, taste, avoidIngredients,
                            ...
                            customRequirements, savingMode, weeklyBudget, dayIndex, mealIndex, newDishName, dishInfo);
                }
            }
            
            return aiService.generateMealPlan(peopleCount, taste, avoidIngredients,
                    favoriteDishes, breakfastWant, lunchWant, dinnerWant,
                    customRequirements, savingMode, weeklyBudget.toString());
        } catch (Exception e) {
            ...
            return buildPlan(peopleCount, taste, available, favoriteDishes, breakfastWant, lunchWant, dinnerWant, 
                    customRequirements, savingMode, weeklyBudget, dayIndex, mealIndex, newDishName);
        }
    }
    ...
    private String generatePlanWithCustomDish(int peopleCount, String taste, List<String> avoidIngredients,
                                               ...
                                               String customRequirements, boolean savingMode, BigDecimal weeklyBudget,
    ...
    private BigDecimal calculateCost(List<Ingredient> used, int peopleCount, boolean savingMode, String mealType) {
        ...
        for (Ingredient ing : used) {
            ...
            if ("L".equals(ing.getUnit())) {
                BigDecimal volume = getVolumeByMealType(mealType, ing.getCategory());
                totalCost = totalCost.add(ing.getPricePerKg().multiply(volume));
            } else {
    ...
    private BigDecimal getVolumeByMealType(String mealType, String category) {
    ...
    public String replaceIngredient(String dishName, List<String> currentIngredients, String replaceIngredient, String taste, int peopleCount) {
        ...
        List<String> newIngredients = new ArrayList<String>(currentIngredients);
        String replacement = "";
        
        if (replaceIngredient != null && !replaceIngredient.isEmpty()) {
            int index = newIngredients.indexOf(replaceIngredient);
            if (index >= 0) {
                Ingredient original = allIngredients.stream()
                        .filter(i -> i.getName().equals(replaceIngredient))
                        .findFirst().orElse(null);
                
                if (original != null) {
                    String category = original.getCategory();
                    List<Ingredient> alternatives = allIngredients.stream()
                            .filter(i -> !i.getName().equals(replaceIngredient) && 
                                    i.getCategory() != null && i.getCategory().contains(category))
                            .sorted(Comparator.comparing(Ingredient::getPricePerKg))
                            .collect(Collectors.toList());
                    
                    if (!alternatives.isEmpty()) {
                        replacement = alternatives.get(0).getName();
                        newIngredients.set(index, replacement);
                    }
                }
            }
        } else {
            for (int i = 0; i < newIngredients.size(); i++) {
                String ing = newIngredients.get(i);
                Ingredient original = allIngredients.stream()
                        .filter(item -> item.getName().equals(ing))
                        .findFirst().orElse(null);
                ...
                if (original != null) {
                    ...
                    List<Ingredient> alternatives = allIngredients.stream()
                            .filter(item -> !item.getName().equals(ing) && 
                                    item.getCategory() != null && item.getCategory().contains(category))
                            .sorted(Comparator.comparing(Ingredient::getPricePerKg))
                            .collect(Collectors.toList());
                    
                    if (!alternatives.isEmpty()) {
                        String alt = alternatives.get(0).getName();
                        newIngredients.set(i, alt);
                        if (replacement.isEmpty()) replacement = alt;
                    }
                }
            }
        }
        
        BigDecimal newCost = calculateCostFromNames(newIngredients, peopleCount, false, "午餐");
        String newDishName = generateNewDishName(dishName, replaceIngredient, replacement, newIngredients);
        
        StringBuilder result = new StringBuilder();
        result.append("{\"newIngredients\":[\"").append(String.join("\",\"", newIngredients)).append("\"]");
        result.append(",\"newDishName\":\"").append(newDishName).append("\"");
        result.append(",\"newCost\":").append(newCost).append("}");
        
        return result.toString();
    }
    
    private BigDecimal calculateCostFromNames(List<String> ingredientNames, int peopleCount, boolean savingMode, String mealType) {
        BigDecimal ratio = savingMode ? new BigDecimal("0.7") : BigDecimal.ONE;
        BigDecimal totalCost = BigDecimal.ZERO;
    ...
    private String generateNewDishName(String originalName, String replaced, String replacement, List<String> newIngredients) {
        if (replacement != null && !replacement.isEmpty()) {
            return originalName.replace(clean(replaced), clean(replacement));
        }
        return clean(newIngredients.get(0)) + clean(newIngredients.get(1)) + "小炒";
    }
}